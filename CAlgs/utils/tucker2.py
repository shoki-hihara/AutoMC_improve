...
class Tucker2DecomposedLayer():
    def __init__(self, layer, layer_name, ranks=None):
        ...
        if '__getitem__' in dir(self.layer):
            self.ranks = estimate_rank_for_compression_rate(
                (self.layer[1].out_channels, self.layer[1].in_channels, *self.kernel_size), rate=ranks)
        else:
            self.ranks = estimate_rank_for_compression_rate(
                (self.cout, self.cin, *self.kernel_size), rate=ranks)
        ...
        for j, (w, b) in enumerate(zip(weights, biases)):
            self.new_layers.__getattr__(
                '{}-{}'.format(self.layer_name, j)).weight.data = w
            if b is not None:
                self.new_layers.__getattr__(
                    '{}-{}'.format(self.layer_name, j)).bias.data = b
            else:
                self.new_layers.__getattr__(
                    '{}-{}'.format(self.layer_name, j)).bias = None
    ...
    def create_new_layers(self):
        ...
        layers.append(nn.Conv2d(in_channels=self.cin,
                      out_channels=self.ranks[1], kernel_size=(1, 1)))

        layers.append(nn.Conv2d(in_channels=self.ranks[1], out_channels=self.ranks[0],
                      kernel_size=self.kernel_size, groups=1, padding=self.padding, stride=self.stride))

        layers.append(nn.Conv2d(
            in_channels=self.ranks[0], out_channels=self.cout, kernel_size=(1, 1)))
    ...
    def get_tucker_factors(self):
        ...
        if '__getitem__' in dir(self.layer):
            ...
            U_cin_old = np.array(torch.transpose(
                w_cin_old.reshape(w_cin_old.shape[:2]), 1, 0))
            U_cout_old = np.array(w_cout_old.reshape(w_cout_old.shape[:2]))

            w_cin = U_cin_old.dot(U_cin)
            w_cout = U_cout_old.dot(U_cout)

        w_cin = torch.FloatTensor(np.reshape(
            w_cin.T, [self.ranks[1], self.cin, 1, 1])).contiguous()
        w_core = torch.FloatTensor(np.reshape(
            w_core, [self.ranks[0], self.ranks[1], *self.kernel_size])).contiguous()
